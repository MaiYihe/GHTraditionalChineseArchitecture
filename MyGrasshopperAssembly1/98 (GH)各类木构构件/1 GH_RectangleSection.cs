...
{
    public class GH_RectangleSection : GH_Component
    {
        /// <summary>
        /// Initializes a new instance of the MyComponent1 class.
        /// </summary>
        public GH_RectangleSection()
          : base("GH_RectangleSection", "Nickname",
              "Description",
        ...
        protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
        {
            ...
            pManager.AddNumberParameter("高", "H", "...", GH_ParamAccess.item);
            pManager.AddNumberParameter("宽", "B", "...", GH_ParamAccess.item);
        ...
        protected override void RegisterOutputParams(GH_Component.GH_OutputParamManager pManager)
        {
            ...
            pManager.AddGenericParameter("顶部线", "DWJX", "...", GH_ParamAccess.list);
            pManager.AddGenericParameter("中部线", "DWJX", "...", GH_ParamAccess.list);
            pManager.AddGenericParameter("底部线", "DWJX", "...", GH_ParamAccess.list);
        }
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            //创建图元信息与边界条件
            RectangleSectionTimber rectangleSectionTimber = new RectangleSectionTimber();
            ...
            ////图元信息，在XY平面上创建一项
            //创建方剖面尺寸
            SquareProfile squareProfile = new SquareProfile(expansion0, expansion1, height, width);
            //取一项,在XY平面上创建SquareProfile
            if (baseCurves.Count == 0 || baseCurves[0] == null)
            {
                this.AddRuntimeMessage(GH_RuntimeMessageLevel.Warning, "未输入定位基线！");
                return;
            }
            rectangleSectionTimber = new RectangleSectionTimber(squareProfile, baseCurves[0].curveLength, name);

            if (rectangleSectionTimber.isExist == false)
            {
                this.AddRuntimeMessage(GH_RuntimeMessageLevel.Warning, "参数错误！");
                return;
            }
            //获取到CircleSectionTimber的几何图元、标注辅助线、定位平面、尺寸信息描述
            Brep timberBrep = rectangleSectionTimber.TimberBrep;
            AnnotationGuideCurves annotationGuidecurves = rectangleSectionTimber.AnnotationGuideCurves;
            Plane referncePlane = rectangleSectionTimber.baseCurveInfluenced.referencePlane;
            String description = rectangleSectionTimber.description;
            //将获取到的信息赋予geometryInformation
            geometryInformation = new GeometryInformation(timberBrep, annotationGuidecurves, referncePlane, description, rectangleSectionTimber.baseCurveInfluenced);
            ////位移成为多项
            //得到XY平面到各项的Transform
            List<Transform> transforms = baseCurves.Select(curve => Transform.PlaneToPlane(Plane.WorldXY, curve.extrudePlane)).ToList();

            //上浮/中心/下沉
            Vector3d v = rectangleSectionTimber.baseCurveInfluenced.positiveDirection;//位移矢量
            Double dis = rectangleSectionTimber.squareProfile.h / 2;
            if (state != null)
            {
                if (state == "上浮")
                {
            ...
            {
                middleBaseCurve = rectangleSectionTimber.baseCurveInfluenced.Duplicate();
                topBaseCurve = rectangleSectionTimber.baseCurveInfluenced.Duplicate();
                bottomBaseCurve = rectangleSectionTimber.baseCurveInfluenced.Duplicate();
            }
            else
            {
                middleBaseCurve = rectangleSectionTimber.baseCurve.Duplicate();
                topBaseCurve = rectangleSectionTimber.baseCurve.Duplicate();
                bottomBaseCurve = rectangleSectionTimber.baseCurve.Duplicate();
            }
        ...
        {
            get { return new Guid("D8771618-40B4-4B3E-8EE0-D11FFF9EDB12"); }
        }
    }
}