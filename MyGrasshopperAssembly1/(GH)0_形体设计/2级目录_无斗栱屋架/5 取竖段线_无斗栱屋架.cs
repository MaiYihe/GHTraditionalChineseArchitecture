...
{
    public class 取竖段线_无斗栱屋架 : GH_Component
    {
        /// <summary>
        /// Initializes a new instance of the 取段线 class.
        /// </summary>
        public 取竖段线_无斗栱屋架()
          : base("取竖段线_无斗栱屋架", "Nickname",
              "Description",
              "Yhm Toolbox", "形体设计")
        {
        }
        ...
        {
            pManager.AddGenericParameter("竖段线", "HDX", "", GH_ParamAccess.list);
        }
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            if (index >= columnVertex.Count / 2)
            {
                this.AddRuntimeMessage(GH_RuntimeMessageLevel.Warning, "索引超出范围！");
                return;
            }
            curve0 = GetVerticalDCurve(columnVertexNeg, index);
            curves.Add(curve0);
            if (isMirror)
            {
                curve1 = GetVerticalDCurve(columnVertexNeg, -index);
                curves.Add(curve1);
            }
        ...
        private Curve GetVerticalDCurve(NegIndexList<Point3d> columnVertexNeg, int index)
        {
            Curve curve = null;
            Curve YZplaneLine = null;

            Point3d startPoint = columnVertexNeg[index];
            Point3d endPoint = Point3d.Unset;
            if (Math.Abs(index) >= columnVertexNeg.Count / 2) return null;
            if (index >= 0)
            {
            ...
            curve.Reverse();
            Plane tmpP = new Plane(startPoint, Plane.WorldYZ.XAxis, Plane.WorldYZ.YAxis);
            YZplaneLine = Curve.ProjectToPlane(curve, tmpP);
        ...
        {
            get { return new Guid("E5DF24B0-5927-42B6-B235-37CF1E098D5C"); }
        }
    }
}